...
The graph is not exactly similar to the original one, but it looks good. I will try to work on more edits change the x-axis and the dashed line text and the colored lines more visually appealing (Maybe plotting the averages?).
...
covid_summary <- covid_infected %>%
  mutate(month = floor_date(as.Date(start_date, format = "%Y-%m-%d"), "month")) %>% #Make the date in consistent format and mutate the `start_date` variable so it's named as `month`
  ...
  summarise( #Create the average percent by month, and round it to two decimal places (applies to the following 4 lines of code)
    avg_very_percent = round(mean(very_percentage, na.rm = TRUE), 2),
    avg_somewhat_percent = round(mean(somewhat_percentage, na.rm = TRUE), 2),
    avg_not_very_percent = round(mean(not_very_percentage, na.rm = TRUE), 2),
    avg_not_at_all_percent = round(mean(not_at_all_percentage, na.rm = TRUE), 2)
  ) %>%
