...
Based on @fig-boxplot, we can see that individuals that work on agriculture have the greatest height and there is not much variation in range. However, individuals that work on industry are the ones that have a lot of variation and most of them are in the 25th percentile. We cannot infer much about individuals that work for government, or occupation or in other professions, since there is just one data point for these. However, it seems that the greatest height are at individuals that work doing household chores.
...
@fig-plot shows that there appears to be an association between age and weight. Based on the linear model, there could be an increase of weight by age, since biologically we cannot say that age increases by weight.
...
In @tbl-resulttable3 we can observe a model fit to predict Height as a function of age and occupation. The model explains the interactions, however none of the occupations and/or age are statistically significant based on the p-value.
